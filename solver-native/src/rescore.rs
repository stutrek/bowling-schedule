...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: rescore [--league summer|summer-fixed] <weights.json> <dir> [dir2 ...]");
    ...
    match league {
        ...
        "summer-fixed" => rescore_summer_fixed(weights_path, dirs),
        other => {
...
fn rescore_summer_fixed(weights_path: &str, dirs: &[&str]) {
    let weights_str = fs::read_to_string(weights_path)
        .unwrap_or_else(|e| panic!("Failed to read {}: {}", weights_path, e));
    let w8: summer_fixed::FixedWeights = serde_json::from_str(&weights_str)
        .unwrap_or_else(|e| panic!("Failed to parse {}: {}", weights_path, e));
    ...
    for dir_arg in dirs {
        ...
        for entry in entries {
            ...
            let sched = match summer_fixed::parse_fixed_tsv(&content) {
                Some(s) => s,
                None => { eprintln!("Warning: could not parse {}", path.display()); errors += 1; continue; }
            };

            checked += 1;
            let cost = summer_fixed::evaluate_fixed(&sched, &w8);
            let filename = path.file_name().unwrap().to_string_lossy().to_string();
            let old_score = filename.get(..4).and_then(|s| s.parse::<u32>().ok());
            let new_score = cost.total;
            ...
            match old_score {
                ...
                Some(old) => {
                    ...
                    match fs::rename(&path, &new_path) {
                        Ok(()) => {
                            eprintln!("  {} -> {} (was {}, now {} | {})", filename, new_filename, old, new_score, summer_fixed::fixed_cost_label(&cost));
                            renamed += 1;
                        }
                        Err(e) => { eprintln!("  ERROR renaming {}: {}", filename, e); errors += 1; }
                    }
                ...
                None => {
                    eprintln!("  {} score={} ({})", filename, new_score, summer_fixed::fixed_cost_label(&cost));
                }
            }
        }
    }
